...
export default function AllBrands() {
  ...
  useEffect(() => {
    ...
    try {
        const response = await axios.post(`${base_domain}/all_brands/`);
  ...
  return (
    ...
    <div className="grid max-sm:grid-cols-2 max-lg:grid-cols-3 grid-cols-6 gap-2 mt-8">
     {brands.map((data, index)=>(
       <Link key={data.id} className='w-full h-[102px] p-[20px] flex justify-center items-center bg-white rounded border border-gray-50 hover:shadow-md duration-75 ease-in-out' to={`/brand-page/${data.slug}`} >
